...
# ---- X‑ray wavelength constants (Å) ----
CU_RAD = 1.5406   # Cu Kα
CO_RAD = 1.79   # fallback (change if you really use Co radiation)
# %%
app = Flask(__name__)
CORS(
    app,
...
def process_data(df):
    ...
    # Safety‑check: drop NaNs generated by invalid arcsin domain
    mask = ~np.isnan(H) & ~np.isinf(H)
    H, intensity = H[mask], intensity[mask]
